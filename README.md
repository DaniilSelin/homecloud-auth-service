...
Для тестирования без реального файлового сервиса используйте mock клиент:

```bash
# Установить переменную окружения для использования mock
...
chmod +x test_registration.sh test_with_mock.sh

# Запустить тестирование с mock
./test_with_mock.sh
```

### Требования для продакшена

В продакшене файловый сервис должен быть доступен, иначе регистрация пользователей будет невозможна. Убедитесь, что:

1. Файловый сервис запущен и доступен по адресу, указанному в конфигурации
2. gRPC порт файлового сервиса открыт и доступен
3. Сетевое соединение между сервисами стабильно
...
### Интеграция с файловым сервисом

- **Обязательное создание папки**: При регистрации пользователя автоматически создается домашняя директория в файловом хранилище
- **Атомарность регистрации**: Если папка не создается, регистрация считается неуспешной и пользователь не создается в базе данных
- **gRPC интеграция**: Взаимодействие с файловым сервисом происходит через gRPC
